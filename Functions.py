...
class Dataset_epoch(Data.Dataset):
    ...
    def __init__(self, names, seg_name, norm=False):
        """Initialization"""
        ...
        self.seg_pair = list(itertools.permutations(seg_name, 2))
    ...
    def __getitem__(self, step):
        ...
        seg_B = load_4D(self.seg_pair[step][1])
        ...
        if self.norm:
            ...
            return torch.from_numpy(img_A).float(), torch.from_numpy(img_B).float(), torch.from_numpy(seg_B).float()


class Dataset_mean(Data.Dataset):
    """Characterizes a dataset for PyTorch"""

    def __init__(self, names, seg_name, label_name, norm=False):
        """Initialization"""
        super(Dataset_mean, self).__init__()
        ...
        self.index_pair = list(itertools.permutations(names, 2))
        self.seg_pair = list(itertools.permutations(seg_name, 2))
        self.label_pair = list(itertools.permutations(label_name, 2))
    ...
    def __getitem__(self, step):
        ...
        if self.norm:
            return torch.from_numpy(imgnorm(img_A)).float(), torch.from_numpy(imgnorm(img_B)).float(), torch.from_numpy(
                imgnorm(seg_B)).float(), torch.from_numpy(imgnorm(label_A)).float(), torch.from_numpy(imgnorm(label_B)).float()
        else:
            return torch.from_numpy(img_A).float(), torch.from_numpy(img_B).float(), torch.from_numpy(seg_B).float(), \
...
class Dataset_epoch_validation(Data.Dataset):
    ...
    def __init__(self, imgs, labels, seg_name, norm=False):
        """Initialization"""
    ...
    def __getitem__(self, step):
        ...
        if self.norm:
            return torch.from_numpy(imgnorm(img_A)).float(), torch.from_numpy(imgnorm(img_B)).float(), torch.from_numpy(
                label_A).float(), torch.from_numpy(label_B).float(), torch.from_numpy(imgnorm(seg_B)).float()
        else:
            return torch.from_numpy(img_A).float(), torch.from_numpy(img_B).float(), torch.from_numpy(
                label_A).float(), torch.from_numpy(label_B).float(), torch.from_numpy(seg_B).float()
